...
export async function POST(request: Request) {
  try {
    ...
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(event.id)) {
      return NextResponse.json(
        { success: false, error: "Invalid id format. Use kebab-case (lowercase letters, numbers, hyphens)" },
        { status: 400 }
      );
    }
    ...
    if (existing) {
      return NextResponse.json(
        { success: false, error: `Event with id "${event.id}" already exists` },
        { status: 409 }
      );
    }
    ...
    return NextResponse.json(
      { success: true, message: "Event created successfully", event: data },
      { status: 201 }
    );
  } catch (error) {
