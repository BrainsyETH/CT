...
import type { ZoraBotPost } from "@/lib/types";

/**
 * Zora Bot Cron Handler
 *
 * This endpoint is called by Vercel Cron (every 15 minutes)
 * It checks if the current time matches a posting slot and creates a
 * Zora Content Coin for the event if needed.
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Post to Zora (create Content Coin)
    const result = await postEventToZora(event, postDate, currentSlot.index);

    if (!result.success) {
      console.error("Failed to post to Zora:", result.error);
      return NextResponse.json(
        {
          error: "Failed to post to Zora",
    ...
    // Save to database
    const postRecord: Omit<ZoraBotPost, "id" | "posted_at"> = {
      post_date: postDate,
      ...
      coin_symbol: result.coinSymbol!,
      coin_url: result.coinUrl || null,
    };
    ...
    console.error("Zora cron job error:", error);
    return NextResponse.json(
      {
